...
async function initializeViewer(){
  ...
  installViewerEvents();installHudBehavior();
  setStatus('最初の景色を表示中…');
  const t0=performance.now(),image=await moveToWithRetry(state.route.frames[0].id,3);
  state.initialDisplayMs=performance.now()-t0;state.successfulFrames=1;
  state.currentProjection=String(image?.cameraType||'').toLowerCase()==='spherical'||state.route.frames[0].isPano?'360':'Rectilinear';
...
async function moveToWithRetry(id,maxAttempts=3){
  ...
  for(let i=0;i<maxAttempts;i++){
    try{return await state.viewer.moveTo(id)}
    catch(error){last=error;if(i+1<maxAttempts)await sleep(180*(i+1))}
  }
  throw last;
}

function reverseGuard(target){
  const maxSeen=Number.isFinite(state.maxProgressSeen)?state.maxProgressSeen:currentFrame()?.routeProgress;
  const behind=Number.isFinite(maxSeen)?maxSeen-target.routeProgress:0;
...
async function moveOne(){
  ...
  const guard=reverseGuard(target);
  if(guard){state.cursor++;state.skippedFrames++;state.stopReason=`skip:${guard}`;logFrame();renderJourneyHud();return true}
  const myToken=state.moveToken,held=Number.isFinite(state.userViewOffset)?state.userViewOffset:0;
  state.heldOffset=held;
  const t0=performance.now();
  try{
    const image=await moveToWithRetry(target.id,3);
    if(!state.playing||myToken!==state.moveToken)return false;
    const dt=performance.now()-t0;
    state.lastTransitionMs=dt;state.transitionTimes.push(dt);state.cursor++;state.successfulFrames++;state.consecutiveMoveErrors=0;
    ...
    setFrameMetrics();await readBearing();await gentlyPreservePano(state.pointerActive?state.heldOffset:held);setFrameMetrics();
    state.stopReason='playing';logFrame();return true;
  }catch(error){
    if(!state.playing||myToken!==state.moveToken)return false;
...
function destroyViewer(){clearHudHideTimer();viewerTapStart=null;if(state.viewer){try{state.viewer.remove()}catch{}state.viewer=null}if($('mly'))$('mly').innerHTML='';state.playing=false;state.moveToken++;updatePlayButton()}

function logFrame(){
  const f=currentFrame();
  state.logs.push({step:state.cursor+1,id:f?.id||null,sequenceIndex:f?.sequenceIndex??null,sequenceGap:f?.sequenceGap??null,distanceFromStartM:f?.distanceFromStartM??null,remainingDistanceM:f?.remainingDistanceM??null,stepDistanceM:f?.stepDistanceM??null,routeProgress:state.currentProgress,travelHeading:state.currentTravelHeading,userViewOffset:state.userViewOffset,currentViewHeading:state.currentViewHeading,moveToResolveMs:state.lastTransitionMs,imageTransitionMs:state.lastTransitionMs,actualCadenceMs:state.lastCadenceMs,projection:state.currentProjection,stopReason:state.stopReason});
}
...
async function seekToRatio(ratio){
  ...
  const idx=clamp(Math.round(ratio*(state.route.frames.length-1)),0,state.route.frames.length-1),target=state.route.frames[idx];
  setStatus(`${distanceLabel(target.distanceFromStartM)}へ移動中…`);
  try{
    const image=await moveToWithRetry(target.id,3);state.cursor=idx;state.currentProjection=String(image?.cameraType||'').toLowerCase()==='spherical'||target.isPano?'360':'Rectilinear';state.maxProgressSeen=target.routeProgress;await readBearing();setFrameMetrics();state.stopReason='seeked';logFrame();
  }catch(e){setStatus(`移動失敗: ${e?.message||e}`)}
  renderJourneyHud();showJourneyHud(true);if(was&&!state.completed)playback();
}

function renderDiagnostics(){
  if(!$('diagGrid'))return;
  const f=currentFrame(),rows=[
    ['version',VERSION],
    ...
    ['transition style','61c4 exact / Default + direct moveTo'],['hold after transition',fmt(Number($('speed')?.value)||DEFAULT_CADENCE,0,'ms')],
    ['moveTo resolve',fmt(state.lastTransitionMs,0,'ms')],['avg moveTo resolve',fmt(average(state.transitionTimes),0,'ms')],
    ['actual cadence',fmt(state.lastCadenceMs,0,'ms')],['avg cadence',fmt(average(state.cadenceTimes),0,'ms')],
    ['visible provider','MapillaryJS built-in'],['cache override','none (MapillaryJS default)'],
    ['route cache',state.routeCacheHit?'HIT':'MISS'],['initialDisplayMs',fmt(state.initialDisplayMs,0,'ms')],
...
function diagnosticsText(){
  ...
  return JSON.stringify({
    test:'MapillaryJS Journey',version:VERSION,start:state.start,goal:state.goal,place:state.selectedPlace?.name||null,
    ...
    playbackTiming:'61c4-default-direct-moveTo-plus-hold',
    visualPipeline:'Viewer(accessToken) + imageTiling + TransitionMode.Default + direct moveTo',
    customDataProvider:false,customZoom:false,customTransitionReset:false,customFilter:false,cacheOverride:false,
    hudAutoHideMs:HUD_AUTO_HIDE_MS,hudTapToggle:true,
    elevationSamples:state.route?.elevationProfile?.length||0,
    successfulFrames:state.successfulFrames,skippedFrames:state.skippedFrames,initialDisplayMs:state.initialDisplayMs,
    averageMoveToResolveMs:average(state.transitionTimes),averageImageTransitionMs:average(state.transitionTimes),averageCadenceMs:average(state.cadenceTimes),
    transitionMode:'Default',routeCacheHit:state.routeCacheHit,reverseEvents:state.reverseEvents,viewJumps:state.viewJumps,panoFrames:state.route?.pano||0,
    totalPreparedFrames:state.route?.frames.length||0,stopReason:state.stopReason,setupMs:state.setupMs,frames:state.logs
  },null,2);
}
...
async function fetchElevationBatch(samples,masterSignal,timeoutMs=4500){
  ...
  try{
    const lats=samples.map(f=>f.lat.toFixed(5)).join(','),lngs=samples.map(f=>f.lng.toFixed(5)).join(',');
    const res=await fetch(`${ELEVATION_API}?latitude=${encodeURIComponent(lats)}&longitude=${encodeURIComponent(lngs)}`,{signal:local.signal,cache:'no-store'});if(!res.ok)throw new Error(`標高API ${res.status}`);const j=await res.json();if(!Array.isArray(j.elevation)||j.elevation.length!==samples.length)throw new Error('標高データ不足');return j.elevation.map(Number);
...
async function loadElevationProfileFast(route){
  ...
  try{
    ...
    let ascent=0,descent=0;for(let i=1;i<profile.length;i++){const d=profile[i].elevation-profile[i-1].elevation;if(d>0)ascent+=d;else descent-=d}state.totalAscentM=ascent;state.totalDescentM=descent;state.durationSec=estimateDurationSec(route.totalDistanceM,ascent);return route.elevations;
  }catch(error){if(error?.name==='AbortError')throw error;if(gen===elevationGeneration&&route===state.route){state.elevationError=String(error?.message||error);route.elevationProfile=[];route.elevations=[];state.totalAscentM=0;state.totalDescentM=0;state.durationSec=estimateDurationSec(route.totalDistanceM,0)}return[]}
...
function renderElevationChartFast(){
  const svg=$('elevationChart'),route=state.route;if(!svg)return;const profile=route?.elevationProfile?.length?route.elevationProfile:(route?.frames||[]).filter(f=>Number.isFinite(f.elevation)).map(f=>({distanceFromStartM:f.distanceFromStartM,elevation:f.elevation}));
...
async function prepareRouteFromEndpoints61c4(){
  ...
  try{
    ...
    const routeRef=state.route;loadElevationProfileFast(routeRef).then(()=>{if(state.route!==routeRef||state.stage!=='trip')return;resetTripTiming();renderTripTimes();renderElevationChartFast()}).catch(()=>{});state.setupMs=performance.now()-setupStart;plannerStatus('ルート確定','旅を開始できます。標高は軽量サンプルで取得します');
  }catch(error){state.stage='search';syncPlannerUI();$('routeSearchTitle').textContent='ルートが見つかりませんでした';$('routeSearchHint').textContent=String(error?.message||error);$('routeSearchBar').style.width='0';$('routeSearchRetry').hidden=false;plannerStatus('出発地点を調整','少し場所を変えて再検索できます')}
}
function updateTripReadyState61c4(){if(!$('tripStart'))return;$('tripStart').disabled=!state.route}
...
function ensureTripEditButton(){
  const panel=document.querySelector('#tripSheet .trip-panel');if(!panel||$('tripEditStart'))return;panel.style.position='relative';const head=panel.querySelector('.trip-head');if(head)head.style.paddingRight='42px';const btn=document.createElement('button');btn.id='tripEditStart';btn.type='button';btn.textContent='×';btn.setAttribute('aria-label','出発地点を選び直す');Object.assign(btn.style,{position:'absolute',right:'10px',top:'8px',width:'34px',height:'34px',minHeight:'34px',padding:'0',borderRadius:'50%',fontSize:'20px',lineHeight:'30px',zIndex:'3',background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.14)',color:'#fff'});btn.addEventListener('click',returnToStartSelectionFromTrip);panel.appendChild(btn);
}
try{loadElevationProfile=loadElevationProfileFast}catch{}
try{renderElevationChart=renderElevationChartFast}catch{}
try{prepareRouteFromEndpoints=prepareRouteFromEndpoints61c4}catch{}
try{updateTripReadyState=updateTripReadyState61c4}catch{}
ensureTripEditButton();

async function startJourney(){
  if(!state.route)return;cancelElevationProfile();state.preloadGeneration++;state.departureTime=new Date();state.durationSec=estimateDurationSec(state.route.totalDistanceM,state.totalAscentM);state.arrivalTime=new Date(state.departureTime.getTime()+state.durationSec*1000);$('tripSheet').hidden=true;$('planner').hidden=true;$('viewerScreen').hidden=false;$('arrivalOverlay').hidden=true;$('viewerRouteLabel').textContent=`${state.selectedPlace?.name||'到着地'} ・ ${distanceLabel(state.route.totalDistanceM)}`;await initializeViewer();renderJourneyHud();showJourneyHud(true);playback();
}
function backToPlanner(){const completed=state.completed;pausePlayback('planner',true);destroyViewer();$('viewerScreen').hidden=true;$('planner').hidden=false;$('arrivalOverlay').hidden=true;if(completed){resetPlanner();try{state.map?.jumpTo({center:[135.7594,35.0068],zoom:12.5,bearing:0,pitch:0})}catch{}}state.map?.resize()}
