...
const ROUTE_INDEX_MARGINS=[350,900];
const ROUTE_INDEX_ENDPOINT_LIMIT=1200;
const ROUTE_INDEX_CANDIDATE_MAX=6;
const ROUTE_INDEX_WAIT_MS=2200;
let routeIndexToken='';
...
async function locateEndpoints(ids){
  ...
  const count=Math.min(22,ids.length),indexes=[];for(let i=0;i<count;i++)indexes.push(Math.round(i*(ids.length-1)/Math.max(1,count-1)));
...
async function evaluateSequence(sequenceId,hint={}){
  try{
    ...
    if(!loc||loc.startD>ROUTE_INDEX_ENDPOINT_LIMIT||loc.goalD>ROUTE_INDEX_ENDPOINT_LIMIT||loc.startIndex===loc.goalIndex)return null;
    ...
    for(const id of chosen){
      const m=metas.get(id),p=pointOf(m);if(!p)continue;
      frames.push({id,sequenceId,lat:p.lat,lng:p.lng,routeProgress:0,routeDistance:0,isPano:!!m?.is_pano,compass:Number.isFinite(+m?.computed_compass_angle)?+m.computed_compass_angle:(Number.isFinite(+m?.compass_angle)?+m.compass_angle:null)});
    }
...
function mapillaryIndexFeatureId(feature){
  return String(feature?.properties?.id??feature?.properties?.sequence_id??feature?.properties?.sequenceId??feature?.id??'').trim();
}
...
function distanceToLineMeters(point,coords){
  ...
  for(let i=0;i<coords.length-1;i++){
    ...
    const vx=bx-ax,vy=by-ay,den=vx*vx+vy*vy,t=den?clamp(-(ax*vx+ay*vy)/den,0,1):0,x=ax+vx*t,y=ay+vy*t;
...
function indexBounds(marginM){
  ...
  const west=Math.min(state.start.lng,state.goal.lng)-lngPad,east=Math.max(state.start.lng,state.goal.lng)+lngPad;
  const south=Math.min(state.start.lat,state.goal.lat)-latPad,north=Math.max(state.start.lat,state.goal.lat)+latPad;
...
async function waitForRouteIndex(timeout=ROUTE_INDEX_WAIT_MS){
  const start=performance.now();await sleep(40);
  while(performance.now()-start<timeout){try{if(state.map.isSourceLoaded(MLY_INDEX_SOURCE))return true}catch{}await sleep(60)}
  return false;
}
function collectIndexCandidates(features,margin){
  ...
  for(const feature of features||[]){
    ...
    for(const line of geometryLines(feature.geometry)){
      item.startDistance=Math.min(item.startDistance,distanceToLineMeters(state.start,line));
      item.goalDistance=Math.min(item.goalDistance,distanceToLineMeters(state.goal,line));
      item.segments++;
    }
  }
  const limit=Math.max(ROUTE_INDEX_ENDPOINT_LIMIT,margin+250),out=[];
  for(const item of groups.values()){
    if(item.startDistance>limit||item.goalDistance>limit)continue;
    item.score=item.startDistance+item.goalDistance-Math.min(80,item.segments*3);out.push(item);
  }
...
async function queryRouteIndex(margin){
  ...
  updateRouteSearchCard?.(`Mapillary撮影ルート索引を確認中… 周囲 ${margin}m`,Math.min(58,16+margin/24));
  ...
  if(!features.length){
    await Promise.race([new Promise(resolve=>state.map.once('idle',resolve)),sleep(550)]);
    try{features=state.map.querySourceFeatures(MLY_INDEX_SOURCE,{sourceLayer:'sequence'})||[]}catch{}
  }
  return{features,candidates:collectIndexCandidates(features,margin)};
}
async function findRouteIndexCandidates(){
  const t0=performance.now();state.routeSearchCandidates=0;
  for(const margin of ROUTE_INDEX_MARGINS){
    const result=await queryRouteIndex(margin);
    if(result.candidates.length){state.routeSearchCandidates=result.candidates.length;state.routeSearchMs=performance.now()-t0;return result.candidates}
  }
  state.routeSearchMs=performance.now()-t0;return[];
}
async function resolveRouteBetweenPoints(){
  ...
  const before=state.graphRequestCount,t0=performance.now(),candidates=await findRouteIndexCandidates();
  if(!candidates.length)throw new Error('Mapillary撮影ルート索引に、出発地と到着地を結ぶ同一sequenceが見つかりませんでした');
  updateRouteSearchCard?.(`候補 ${candidates.length}本。MapillaryJS用の画像順序を確認しています…`,66);
  let best=null;
  for(let i=0;i<Math.min(4,candidates.length);i++){
    updateRouteSearchCard?.(`候補 ${i+1}/${Math.min(4,candidates.length)} の進行方向を確認中…`,68+i*6);
    best=await evaluateSequence(candidates[i].sequenceId,candidates[i]);if(best)break;
  }
  state.routeSearchApiRequests=state.graphRequestCount-before;
  if(!best)throw new Error('撮影ルートは見つかりましたが、MapillaryJSで再生できる出発→到着区間を確定できませんでした');
...
async function loadElevationProfile(route){
  ...
  try{
    ...
    for(let i=0;i<frames.length;i++){frames[i].elevation=Number(j.elevation[i]);if(i){const d=frames[i].elevation-frames[i-1].elevation;if(d>0)ascent+=d;else descent-=d}}
    ...
    state.elevationError=String(e?.message||e);route.elevations=[];state.totalAscentM=0;state.totalDescentM=0;state.durationSec=estimateDurationSec(route.totalDistanceM,0);return[];
