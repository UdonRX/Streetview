/* Streetview Journey Phase 2 coverage map shell */
const SHELL_CACHE='streetview-shell-phase2-map-v1';
const IMAGE_CACHE='streetview-images-v0.1.0';
const SHELL=['/','/index.html','/map.css','/map.js','/journey.html','/styles.css?v=0.1.6','/travel-axis.js?v=0.1.30','/travel-axis-worker.js?v=0.1.30','/app.js?v=0.1.30','/diagnostics.js?v=0.1.30','/motion-worker.js?v=0.1.25','/manifest.webmanifest'];
const IMAGE_LIMIT=80;
const MAP_TILE_HOSTS=new Set(['tiles.openfreemap.org','api.openstreetcam.org','tiles.mapillary.com']);
...
self.addEventListener('fetch',event=>{
  ...
  if(MAP_TILE_HOSTS.has(url.hostname)){event.respondWith(fetch(request));return;}
  if(request.destination==='image'&&url.origin!==self.location.origin){if(request.mode==='cors'){event.respondWith(fetch(request));return;}event.respondWith((async()=>{const cache=await caches.open(IMAGE_CACHE),hit=await cache.match(request);if(hit)return hit;const response=await fetch(request);if(response.ok||response.type==='opaque'){await cache.put(request,response.clone());trimImages(cache).catch(()=>{});}return response;})());return;}
