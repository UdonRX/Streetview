...
function patchRawRuntimeSource(text){
  let out=text;
  out=out.replace('/* Streetview Journey raw/analysis transport split v0.1.46 */','/* Streetview Journey raw/analysis transport split v0.1.48 strict cache continuity */').replace("const VERSION='0.1.46';","const VERSION='0.1.48';");
  out=out.replace(
    ...
    "function rawAhead(){const actual=Number(window.JourneyEngine?.contiguousRenderableAhead?.(currentIndex(),64));return Number.isFinite(actual)?Math.max(0,actual):0}"
  );
  out=out.replace(/  function rawVariantFor\(raw,index\)\{[\s\S]*?\n  \}\n\n  function taskKey/,`  function rawVariantFor(raw,index){
    ...
    const f=Number.isFinite(index)?frameAt(index):null,id=String(f?.id||''),nextRequired=Number.isFinite(index)&&index===currentIndex()+1;
    ...
    if(nextRequired&&id&&u256&&!variantFailed(id,'256-continuity'))return{url:u256,variant:'256-continuity'};
  ...
  out=out.replace("rawVariant:'256 continuity lane + 1024 quality lane + source fallback'","rawVariant:'next-frame 256 strict + 1024 quality after continuity + source last'");
  return out;
}
async function rawRuntimeWithContinuityPatch(request){const response=await sourceResponse(request,'/raw-runtime.js?v=0.1.48');const patched=patchRawRuntimeSource(await response.text());return textResponse(response,patched,'application/javascript; charset=utf-8');}
...
function patchAppSource(text){
  ...
  let recoverableImageError=false,recoverableErrorIndex=null;
  ...
  function restoreDisplayLayer(i,im){const actual=im||renderableFrame(i),src=String(actual?.currentSrc||actual?.src||route[i]?.raw256Url||route[i]?.url||'');if(!src)return;try{if(ui.a){ui.a.src=src;ui.a.classList.add('is-visible');ui.a.style.opacity='1'}if(ui.b){ui.b.classList.remove('is-visible');ui.b.style.opacity='0'}}catch{}}
  function showRecoverableImageError(error,index){recoverableImageError=true;recoverableErrorIndex=Number.isFinite(index)?index:currentPlaybackIndex;if(ui.err)ui.err.textContent=error?.message||'画像を読み込めませんでした。再取得中です';if(ui.error)ui.error.hidden=false;if(ui.start)ui.start.hidden=true;if(ui.back)ui.back.hidden=false}
  function recoverPlaybackUI(index=null,im=null){if(!recoverableImageError)return;let restoreIndex=hasRenderableFrame(currentPlaybackIndex)?currentPlaybackIndex:(Number.isFinite(index)?index:recoverableErrorIndex);const actual=im||renderableFrame(restoreIndex);if(actual)restoreDisplayLayer(restoreIndex,actual);recoverableImageError=false;recoverableErrorIndex=null;if(ui.error)ui.error.hidden=true;if(ui.canvas)ui.canvas.style.opacity='1';if(ui.back)ui.back.hidden=false}`
  );
  ...
  function routeMetrics(){const s=streamState(),available=route.length,streamFrameCount=Array.isArray(s?.frames)?s.frames.length:available,total=Math.max(available,streamFrameCount,Number(s?.total)||0),metrics={available,routeLength:available,total,streaming:!!s?.active,streamAvailable:available,streamFrameCount};if(s){s.available=available;s.routeLength=available;s.total=total}return metrics}
  function syncPlaybackAvailability(){const metrics=routeMetrics(),p=window.__journeyPlaybackState;if(p)Object.assign(p,metrics);return metrics}
  ...
  out=out.replace("diag.lastRenderPath=path;try{window.dispatchEvent(new CustomEvent('journey-frame-presented'","diag.lastRenderPath=path;recoverPlaybackUI(i,im||st?.image||null);try{window.dispatchEvent(new CustomEvent('journey-frame-presented'");
  out=out.replace("const im=renderReadyCache.get(f.url)||await loadRender(f.url);drawRaw(0,im);","const im=renderableFrame(0)||(await waitForRawFrame(0,token)).image;rememberRenderable(0,im);recoverPlaybackUI(0,im);drawRaw(0,im);");
  ...
  out=out.replace("].forEach(c=>c.clear());Object.assign(diag,", "].forEach(c=>c.clear());renderableByIndex.clear();recoverableImageError=false;recoverableErrorIndex=null;Object.assign(diag,");
  ...
  out=out.replace(/  async function waitForRawFrame\(index,pt\)\{[\s\S]*?\n  async function play\(frames\)/,`  async function waitForRawFrame(index,pt){const initialUrl=route[index]?.url;if(!initialUrl)throw new Error('画像URLがありません');const first=renderableFrame(index);if(first)return{image:first,waitedMs:0};const started=performance.now();try{window.dispatchEvent(new CustomEvent('journey-image-wait-start',{detail:{index,ahead:aheadCounts(Math.max(0,index-1))}}))}catch{}let lastError=null;for(let attempt=0;pt===token;attempt++){const ready=renderableFrame(index);if(ready){recoverPlaybackUI(index,ready);const waitedMs=performance.now()-started;try{window.dispatchEvent(new CustomEvent('journey-image-wait-resolved',{detail:{index,waitedMs:Math.round(waitedMs),attempt,ahead:aheadCounts(Math.max(0,index-1))}}))}catch{}return{image:ready,waitedMs}}const url=route[index]?.url;if(!url)throw new Error('画像URLがありません');try{const image=await ensureRaw(index,-3000-Math.min(attempt,500));if(pt!==token)throw new Error('playback-cancelled');rememberRenderable(index,image);recoverPlaybackUI(index,image);const waitedMs=performance.now()-started;try{window.dispatchEvent(new CustomEvent('journey-image-wait-resolved',{detail:{index,waitedMs:Math.round(waitedMs),attempt,ahead:aheadCounts(Math.max(0,index-1))}}))}catch{}return{image,waitedMs}}catch(error){lastError=error;if(pt!==token)throw error;renderCache.delete(url);renderReadyCache.delete(url);enqueueRaw(index,-4000-Math.min(attempt,500));if(attempt>=3&&(attempt===3||attempt%8===3))showRecoverableImageError(error,index);await sleep(Math.min(240,20+attempt*12))}}throw lastError||new Error('playback-cancelled')}
  ...
  out=out.replace("window.__journeyPlaybackState={...(window.__journeyPlaybackState||{}),index:i,available:route.length,total:Math.max(route.length,streamState()?.total||0),streaming:false,complete:true};","window.__journeyPlaybackState={...(window.__journeyPlaybackState||{}),index:i,...routeMetrics(),streaming:false,complete:true};");
  out=out.replace("function showStartError(e){ui.canvas.style.opacity='0';ui.back.hidden=true;ui.err.textContent=e?.message||'不明なエラーが発生しました';","function showStartError(e){const message=e?.message||'不明なエラーが発生しました';recoverableImageError=false;recoverableErrorIndex=null;ui.canvas.style.opacity='0';ui.back.hidden=true;ui.err.textContent=message;");
