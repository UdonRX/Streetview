/* Streetview Journey v0.1.18 iPhone Safari OpenCV Loader */
(() => {
  const LOADER_VERSION = '0.1.18';
  const SOURCES = [
  ...
  function updateApiState(cv) {
    ...
    if (isApiReady(cv)) {
      ...
      state.error = null;
      window.cv = cv;
      window.dispatchEvent(new CustomEvent('journey-opencv-ready', { detail: { version: LOADER_VERSION, source: state.source } }));
      return true;
    }
    return false;
  }

  async function unwrapCv(timeoutMs = 800) {
  ...
  function renderLoaderState() {
    ...
    if (eyebrow && eyebrow.textContent !== 'v0.1.18 PHASE 1.2.1 OPENCV LOADER FIX') {
      eyebrow.textContent = 'v0.1.18 PHASE 1.2.1 OPENCV LOADER FIX';
    }
  ...
  async function loadSource(url, attempt) {
    state.source = url;
    ...
    await loaded;
    return waitUntilReady(18000);
  }

  async function boot() {
    const already = await unwrapCv(50);
